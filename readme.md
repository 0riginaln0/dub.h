...
The feature of Dub is that commands routing & handling are integrated. You can execute arbitrary code at any point or branch during the routing process.
...
```txt
  ...
  plus -- sums up two integers
      expects two integer values e.g. `2 2`
...
int main(int argc, char** argv) {
  ...
  dub_description(&dub, "dub -- CLI app for featuring the dub.h library", NULL);

  if (dub_match(&dub, "version")) {
    dub_description(&dub, "version -- Get the version of the Dub app", help_file);
    printf("Dub version: 1.0.0\n");
  }
  else if (dub_match(&dub, "plus")) {
    dub_description(&dub, "plus -- sums up two integers", help_file);
    int a, b;
    if (dub_parse_integer(&dub, &a) && dub_parse_integer(&dub, &b)) {
      dub_description(&dub, "expects two integer values e.g. `2 2`", help_file);
      printf("%d + %d = %d\n", a, b, a + b);
    }
    else {
      printf("Bro, I need two numbers. Try `dub plus 2 2`\n");
    ...
    dub_description(&dub, "help|-h|--help -- Prints the help menu", help_file);
    printf(
       "dub -- CLI app for featuring the dub.h library\n"
       "  version -- Get the version of the Dub app\n"
       "  plus -- sums up two integers\n"
       "      expects two integer values e.g. `2 2`\n"
       "  help|-h|--help -- Prints the help menu\n"
